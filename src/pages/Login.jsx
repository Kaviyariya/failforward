...
import { TerminalSquare, Mail, Lock } from 'lucide-react';
  ...
  return (
    <div className="max-w-md mx-auto mt-20 animate-fade-in">
      <div className="bg-brand-card p-10 rounded-[2rem] shadow-2xl border border-brand-border relative overflow-hidden">
        <div className="absolute -top-24 -right-24 w-48 h-48 bg-brand-purple/20 blur-[60px] rounded-full pointer-events-none"></div>
        
        <div className="text-center mb-10 relative z-10">
          <TerminalSquare size={48} className="mx-auto text-brand-purple mb-4" />
          <h2 className="text-3xl font-black text-white tracking-tight">Welcome Back</h2>
          <p className="text-brand-textMuted mt-2 font-medium">Log in to your developer account</p>
        </div>
        ...
        <p className="mt-8 text-center text-brand-textMuted text-sm font-medium relative z-10">
          New here? <Link to="/register" className="text-brand-purpleLight hover:text-white transition-colors underline decoration-brand-purple/50 underline-offset-4">Create an account</Link>
        </p>
      </div>
    </div>
  );
